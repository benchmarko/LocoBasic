(function (global, factory) {
    ...
    const workerFn = (parentPort) => {
      ...
      const vm = {
        _commaOpChar: "\u2192",
        // Unicode arrow right (Constants.CommaOpChar)
        _tabOpChar: "\u21D2",
        // Unicode double arrow right (Constants.TabOpChar)
        ...
        _graBackgroundColor: "",
        _graColorsForPens: [],
        // make sure to initialize with resetColorsForPens before usage
        ...
        _pausePromise: void 0,
        _pauseResolvedFn: void 0,
        _timerMap: {},
        _vpos: 0,
        _zone: 13,
        ...
        convert2ControlCodes: (str) => {
          ...
          for (let i = 0; i < str.length; i += 1) {
            ...
            out += String.fromCharCode(code >= 256 && code <= 287 ? code - 256 : code);
          }
          return out;
        },
        ...
        resolvePause: () => {
          if (vm._pauseResolvedFn) {
            vm._pauseResolvedFn("");
            vm._pauseResolvedFn = void 0;
            vm._pausePromise = void 0;
          }
        },
        pauseTimer: (timer) => {
        ...
        resumeTimer: (timer) => {
          const timerObj = vm._timerMap[timer];
          if (timerObj) {
            timerObj.id = timerObj.repeat ? setInterval(() => timerObj.fn(), timerObj.timeout) : setTimeout(() => timerObj.fn(), timerObj.timeout);
          }
        },
        resumeAllTimers: () => {
          for (const timer in vm._timerMap) {
        ...
        flush: () => {
          ...
          if (message) {
            const hasGraphics2 = vm._graOutputGraphicsIndex >= 0;
            vm.postMessage({ type: "flush", message, hasGraphics: hasGraphics2, needCls: vm._needCls });
            vm._needCls = false;
          }
        },
        frame: async () => {
        ...
        graDrawMovePlot: (type, x, y, pen) => {
          ...
          x = Math.round(x);
          y = Math.round(y);
          if (!vm._graGraphicsPathBuffer.length && type !== "M" && type !== "P") {
...
`;
        ...
        graPrintGraphicsText: (text) => {
          ...
          vm._graGraphicsX += text.length * 8;
          vm._graLastEmittedX = vm._graGraphicsX + vm._graOriginX;
        },
        ...
        instr: (str, find, len) => {
          return vm.convert2ControlCodes(String(str)).indexOf(vm.convert2ControlCodes(String(find)), len !== void 0 ? len - 1 : len) + 1;
        },
        ...
        paper: (n) => {
          if (n !== vm._paperValue) {
            ...
            if (vm._paperSpanPos >= 0) {
              if (vm._penSpanPos > vm._paperSpanPos) {
                vm._output += "</span>";
                vm._penSpanPos = -1;
              }
              vm._output += "</span>";
        ...
        pen: (n) => {
          if (n !== vm._penValue) {
            ...
            if (vm._penSpanPos >= 0) {
              if (vm._paperSpanPos > vm._penSpanPos) {
                vm._output += "</span>";
                vm._paperSpanPos = -1;
              }
              vm._output += "</span>";
        ...
        printText: (text) => {
          ...
          vm._output += vm._isTerminal ? text : vm.escapeText(text);
      ...
      const onRun = (code) => {
        ...
        const fnScript = new Function("_o", `"use strict"; return (async () => { ${code} })();`);
        if (!isNodeParentPort) {
          parentPort.removeEventListener("error", errorEventHandler);
        }
        const truncateOutput = (property) => {
          const maxLength = 16777216;
          if (vm[property].length > maxLength) {
