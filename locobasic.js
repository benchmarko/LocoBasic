(function (global, factory) {
    ...
    class Parser {
      constructor(grammarString, semanticsMap, superParser) {
        ...
        this.ohmSemantics = this.ohmGrammar.createSemantics().addOperation("eval", semanticsMap);
      }
      getOhmGrammar() {
  ...
  `
    ...
    const _SemanticsHelper = class _SemanticsHelper {
      constructor() {
        this.lineIndex = 0;
        this.indent = 0;
      ...
      getVariable(name, type = "") {
        ...
        if (_SemanticsHelper.reJsKeyword.test(name)) {
          name = `_${name}`;
        }
        ...
        if (defContextStatus === "") {
          ...
          if (!this.defContextVars.includes(name)) {
            this.createVariableOrCount(name, type);
          }
        }
      ...
      resetParser() {
        ...
        _SemanticsHelper.deleteAllItems(this.variables);
        _SemanticsHelper.deleteAllItems(this.variableScopes);
        _SemanticsHelper.deleteAllItems(this.varLetterTypes);
        this.currentFunction = "";
        this.definedLabels.length = 0;
        _SemanticsHelper.deleteAllItems(this.usedLabels);
        this.dataList.length = 0;
        this.dataIndex = 0;
        _SemanticsHelper.deleteAllItems(this.restoreMap);
        _SemanticsHelper.deleteAllItems(this.instrMap);
        this.isDeg = false;
        this.isTag = false;
        this.defContextStatus = "";
        this.defContextVars.length = 0;
    ...
    _SemanticsHelper.reJsKeyword = /^(arguments|await|break|case|catch|class|const|continue|debugger|default|delete|do|else|enum|eval|export|extends|false|finally|for|function|if|implements|import|in|instanceof|interface|let|new|null|package|private|protected|public|return|static|super|switch|this|throw|true|try|typeof|var|void|while|with|yield)$/;
    let SemanticsHelper = _SemanticsHelper;
...
` + lineList[first];
            lineList[label.last] = lineList[label.last].replace(`${indentStr}  return;`, `${indentStr}}`);
            if (hasAwait) {
              awaitLabels.push(subroutineStart.label);
            }
            subroutineStart = void 0;
          }
        }
        return awaitLabels;
      }
      ...
      const semantics = {
        Program(lines) {
          ...
          if (asyncLabels.size && definedLabels.length) {
...
`;
        ...
        Statements(colons1, stmt, colons2, stmts) {
          if (colons1.children.length) {
            notSupported(colons1);
          }
          if (colons2.child(0)?.children.length > 1) {
            notSupported(colons2.child(0));
          }
        ...
        Call(lit, args) {
          const num = Number(args.asIteration().child(0).eval());
          let result = "";
          switch (num) {
            case 47878:
            // fall through...
        ...
        DefAssign(ident, args, _equal, e) {
          semanticsHelper.setDefContextStatus("start");
        ...
        Defint(_lit, letterRange) {
          const lettersAndRanges = evalChildren(letterRange.asIteration().children);
...
${initialIndent}}`;
        ...
        If_singleLine(_iflit, condExp, thenStat, colons, elseLit, elseStat) {
          ...
          if (colons.child(0)?.children.length) {
...
${initialIndent}}`;
        ...
        Key_def(lit, defLit, num, comma, repeat, comma2, codes) {
          if (num.eval() === "78" && repeat.eval() === "1") {
            const codeList = codes.child(0) ? evalChildren(codes.child(0).asIteration().children) : void 0;
        ...
        On_numGosub(_onLit, e1, _gosubLit, args) {
          ...
          return `([${argumentList.map((label) => `_${label}`).join(",")}]?.[${index} - 1] || (() => undefined))()`;
        },
        ...
        RsxArgs(_comma, args) {
          const argumentList = evalChildren(args.asIteration().children);
          const argumentListNoAddr = argumentList.map(
            (arg) => arg.startsWith("@") ? arg.substring(1) : arg
          );
          const assignList = argumentList.map((arg) => arg.startsWith("@") ? arg.substring(1) : void 0);
          while (assignList.length && assignList[assignList.length - 1] === void 0) {
            assignList.pop();
          }
        ...
        Tab(_lit, _open, num, _close) {
          return `"${TabOpChar}" + String(${num.eval()})`;
        },
        Tag(lit, stream) {
        ...
        DimArrayIdent(ident, _open, indices, _close) {
          ...
          const isMultiDimensional = indicesStr.includes(",");
          ...
          if (isMultiDimensional) {
            instr = "dim";
          } else if (!isStringIdent && semanticsHelper.getVarLetterType(identStr) === "I") {
            instr = "dim1i16";
          } else {
            instr = "dim1";
          }
        ...
        decimalValue(value) {
          const valueStr = value.sourceString.replace(/^(-?)(0+)(\d)/, "$1$3");
        ...
        string(_quote1, e, quote2) {
          const str = e.sourceString.replace(/\\/g, "\\\\");
...
`
      ...
      compileScript(script) {
        ...
        const preprocessedScript = Core.expandNextStatements(script, semanticsHelper);
...
` + VmMessageHandler.describeError(this.code, lineno - 2, colno) + "\n" + message;
    ...
    class NodeParts {
      ...
      getNodeModule(module2) {
        if (!this.loadedNodeModules[module2]) {
          this.loadedNodeModules[module2] = require(module2);
        }
        return this.loadedNodeModules[module2];
      }
      getNodePath() {
      ...
      async nodeReadFile(name) {
        ...
        if (!module) {
          const module2 = this.getNodeModule("module");
          this.modulePath = module2.path || "";
          if (!this.modulePath) {
      ...
      fnOnKeypress(_chunk, key) {
        if (key) {
          ...
          if (key.name === "c" && key.ctrl === true) {
            ...
            this.getNodeVmMain().stop();
          } else if (keySequenceCode === 13 || keySequenceCode >= 32 && keySequenceCode <= 128) {
